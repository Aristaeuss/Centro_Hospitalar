...
<html lang='en'>
  ...
  <body>
    <div class="main">
      ...
      <div  class="pageIdentifier">PÁGINA DE CONFIRMAÇÃO DE SENHA</div>
      <div class="transparentCardBar">
              </div>
      <div class="confirmedMessage">
      <h3>Foi criada a senha número ${ticket.getId()} para ${ticket.getScheduledAppointment().getPatient().getPreferredName()}</h3>
      <h4>Detalhes da consulta:</h4>
      <p>Médico: Dr(a) ${ticket.getScheduledAppointment().getSlot().getDoctor().getPreferredName()}</p>
      <p>Especialidade: ${ticket.getScheduledAppointment().getSlot().getSpecialityName()}</p>
      <p>Data: ${ticket.getScheduledAppointment().getSlot().getDate()}</p>
      <p>Horário: ${ticket.getScheduledAppointment().getSlot().getDateTime().toLocalTime()}</p>
      </div>
    </div>
