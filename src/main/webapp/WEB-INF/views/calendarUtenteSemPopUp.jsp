    ...
    <div class="main">
      ...
      <div  class="pageIdentifier">CALENDÁRIO <br>
            <c:if test="${scheduledAppointmentToCancel!=null}">
              <p>Está a remarcar a consulta do dia ${scheduledAppointmentToCancel.getSlot().getDate()} às ${scheduledAppointmentToCancel.getSlot().getDateTime().toLocalTime()}
                    ...
                    <div class="doctor-list">
                        ...
                        <ul style="list-style-type:none;padding-left: 0rem;">
                            <c:forEach var="doctor" items="${doctorsAvailableForDay}">
                                <li class="doctorItem"><a href="/calendarioUtenteL/${dateSelected}D${doctor.getOrderNumber()}"> Dr. ${doctor.getPreferredName()}</a></li>
                            </c:forEach>
                        </ul>
                        </div>
                    </div>
            ...
            <div class="day-schedule">
              ...
              <div class="hours">
                <c:forEach var="slot" items="${slotsForDoctor}">
                  <c:if test="${scheduledAppointmentToCancel==null}"><a class="hourButton" href="/consultaMarcada/${slot.getId()}">${slot.getDateTime().toLocalTime()}</a></c:if>
                  <c:if test="${scheduledAppointmentToCancel!=null}"><a class="hourButton" href="/remarcarConsultaUtente/${scheduledAppointmentToCancel.getId()}/${slot.getId()}">${slot.getDateTime().toLocalTime()}</a></c:if>
                </c:forEach>
              </div>
            </div>
          </div>
